...
  const saldo     = ingresos - gastos
  ...
  const CATS_INGRESO = ['Independiente / Honorarios','Dividendos ZABÚ','Dividendos RV Sports','Dividendos Las Bombas','Freelance / Consultoría','Arriendo recibido','Ingreso ocasional','Otro']
...
       <div style={{ fontSize:11, color:'var(--text3)', marginBottom:8 }}>Fuente / Categoría</div>
       <div style={{ display:'flex', flexDirection:'column', gap:6, maxHeight:180, overflowY:'auto', paddingRight:4 }}>
         {(tipo==='ingreso'?CATS_INGRESO:CATS_GASTO).map(c => (
           <div key={c} onClick={() => setCat(c)} style={{
             padding:'10px 14px', borderRadius:8, cursor:'pointer', fontSize:13,
             background: cat===c ? (tipo==='ingreso'?'var(--green-dim)':'rgba(224,82,82,0.1)') : 'rgba(255,255,255,0.04)',
             border:`1px solid ${cat===c ? (tipo==='ingreso'?'var(--green-border)':'rgba(224,82,82,0.3)') : 'var(--border)'}`,
             color: cat===c ? (tipo==='ingreso'?'var(--green)':'var(--red)') : 'var(--text2)',
