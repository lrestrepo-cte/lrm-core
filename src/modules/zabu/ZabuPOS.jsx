      ...
      <div style={{
        ...
        <div style={{ overflowY:'auto', padding:isMobile?'12px':'16px 20px', borderRight:isMobile?'none':'1px solid var(--border)', background:'var(--bg)', borderBottom:isMobile?'1px solid var(--border)':'none' }}>
          ...
          {!fasePago ? (
            ...
            <>
              <button onClick={() => setFasePago(false)} style={{ fontSize:12, color:'var(--text3)', background:'none', border:'none', cursor:'pointer', fontFamily:'inherit', marginBottom:14 }}>
                ← Volver a editar
              </button>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:8, marginBottom:12 }}>
                  {[
                    ...
                    <div key={t.id} onClick={() => setEntrega(t.id)} style={{ ...cardBase,
                      ...
                      background: entrega===t.id?t.color+'15':'rgba(255,255,255,0.03)',
                    }}>
              ...
              {entrega && (
                <div className="panel" style={{ marginBottom:12 }}>
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
                    ...
                    <button onClick={() => setPagos(prev=>[...prev,{metodo:'efectivo',monto:''}])} style={{ fontSize:11, padding:'4px 10px', borderRadius:8, cursor:'pointer', background:'rgba(255,255,255,0.06)', border:'0.5px solid var(--border)', color:'var(--text3)', fontFamily:'inherit' }}>
                      + Mixto
                    </button>
                  </div>
                  {pagos.map((p,i) => (
                    <div key={i} style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:8, marginBottom:8 }}>
                      ...
                      <input type="number" value={p.monto} onChange={e=>updatePago(i,'monto',e.target.value)}
                        placeholder={i===0?`${totalPrecio}`:'Monto'}
              ...
              {entrega && pagoCompleto && (
                <button className="btn-green" onClick={confirmar} style={{ fontSize:15, fontWeight:800 }}>
                  ✓ Confirmar · {cop(totalPrecio)}
                </button>
              )}
