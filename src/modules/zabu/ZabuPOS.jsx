...
export default function ZabuPOS({ usuario }) {
  ...
  const confirmar = async () => {
    ...
    // ── DESCUENTO DE INVENTARIO (zabu_lotes) por FIFO ──────────────────────
    // Por cada item vendido: busca su ficha técnica en Supabase, obtiene los
    // ingredientes con gramajes, y descuenta de zabu_lotes el lote más próximo
    // a vencer (FIFO). Si no hay lote suficiente, registra el descuento parcial
    // sin bloquear la venta — la operación no puede detenerse por inventario.
    // El asiento de costo de venta (6140/1430) se genera al final si hay costo.
    try {
      const itemsADescontar = itemsCompletos.length ? itemsCompletos : items
      const { data: todasFichas } = await supabase
        .from('zabu_fichas_tecnicas')
        .select('id, nombre')
      ...
      if (todasFichas && todosLotes) {
        let costoTotalVenta = 0

        for (const item of itemsADescontar) {
          // Mapear categoría del item al nombre de la ficha técnica
          const nombreFicha =
            item.categoria === 'hotdog'     ? 'ZABÚ'
          : item.categoria === 'burger'     ? (item.burger?.nombre || null)
          : item.categoria === 'salchipapa' ? 'Salchipapa ZABÚ'
          : item.categoria === 'fries'      ? (item.friesItem?.nombre || 'Fries Z')
          : item.categoria === 'kids'       ? 'Kids ZABÚ'
          : item.categoria === 'granizado'  ? 'Granizado'
          : item.categoria === 'paleta'     ? 'Paleta Z'
          : null

          if (!nombreFicha) continue

          const ficha = todasFichas.find(f => f.nombre === nombreFicha)
          if (!ficha) continue

          const { data: ingredientes } = await supabase
            .from('zabu_ficha_ingredientes')
            .select('*')
          ...
          for (const ing of ingredientes) {
            const gramos = parseFloat(ing.cantidad) || 0
            if (gramos <= 0) continue

            // Buscar lotes del insumo ordenados por vencimiento (FIFO)
            const lotesInsumo = todosLotes.filter(l =>
              l.producto_nombre.toLowerCase().includes(ing.nombre.toLowerCase().split(' ')[0]) ||
              ing.nombre.toLowerCase().includes(l.producto_nombre.toLowerCase().split(' ')[0])
            )

            let restante = gramos
            for (const lote of lotesInsumo) {
              if (restante <= 0) break
              const disponible = parseFloat(lote.cantidad_actual) || 0
              ...
              // Acumular costo real para el asiento de costo de venta
              costoTotalVenta += aDescontar * (lote.costo_unitario || 0)
              restante -= aDescontar
            }
          }
        }
        ...
        // Débito 6140 Costo de ventas / Crédito 1430 Inventario (producto terminado)
        // Solo se genera si el costo calculado es mayor a $0 para evitar
        // asientos vacíos que ensucian el Diario.
        if (Math.round(costoTotalVenta) > 0) {
          const { data: asientoCosto } = await supabase.from('asientos').insert({
            fecha: new Date().toISOString().split('T')[0],
            descripcion: `Costo de venta ${codigo}`,
            carrito_id: CARRITO_ID,
          }).select().single()
          ...
          if (asientoCosto) {
            await supabase.from('partidas').insert([
              {
                asiento_id: asientoCosto.id, codigo:'6140',
                nombre:'Costo de ventas — insumos consumidos',
                debe: Math.round(costoTotalVenta), haber: 0, carrito_id: CARRITO_ID,
              },
              {
                asiento_id: asientoCosto.id, codigo:'1430',
                nombre:'Producto terminado',
                debe: 0, haber: Math.round(costoTotalVenta), carrito_id: CARRITO_ID,
              },
            ])
          }
        }
      ...
      // El descuento de inventario nunca bloquea la venta —
      // si falla, la orden ya quedó registrada y el cajero puede continuar.
      console.warn('Descuento inventario fallido (no bloquea venta):', errInventario)
    }
