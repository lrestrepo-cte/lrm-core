...
// ─── APP VENDEDOR ─────────────────────────────────────────────────────────────
// Flujo: AperturaTurno → POS con botón "Ver turno" → PanelTurno overlay
function VendedorApp({ usuario, onCerrarSesion }) {
  const [turno,    setTurno]    = useState(null)
  const [verTurno, setVerTurno] = useState(false)
  const { isMobile } = useBreakpoint()

  // Sin turno abierto → pantalla de apertura
  if (!turno) {
  ...
  return (
    <div className="app-root">
      ...
      {/* Panel turno como overlay (sin desmontar el POS) */}
      {verTurno && (
        <PanelTurno
          turno={turno}
          usuario={usuario}
          ...
          onTurnoCerrado={() => {
            setVerTurno(false)
            setTurno(null) // Vuelve a apertura para el siguiente turno
          }}
        />
      )}
...
// ─── APP COCINA ───────────────────────────────────────────────────────────────
// Flujo directo → Comandero con botón salir
...
export default function App() {
  // Estado inicial limpio — sin leer localStorage para evitar el bug de persistencia
  ...
  // ── HANDLERS ──
  const handleLogin = (u) => {
    localStorage.setItem('lrm_usuario', JSON.stringify(u))
    setUsuario(u)
  }
  ...
  // Vendedor → flujo turno + POS (sin sidebar)
  if (usuario.rol === 'vendedor') {
    return <VendedorApp usuario={usuario} onCerrarSesion={cerrarSesion} />
  }

  // Cocina → Comandero directo (sin sidebar)
  if (usuario.rol === 'cocina') {
    return <CocinaApp usuario={usuario} onCerrarSesion={cerrarSesion} />
  }

  // CEO → LRM Core completo con sidebar
  ...
  const renderContenido = () => {
    if (isZabu) {
      return (
        <ZabuApp
          navExterno={zabuNav}
          onNavChange={(id) => { setZabuNav(id); setNavActivo(id) }}
          usuario={usuario}
        />
      )
    }
    switch (navActivo) {
      ...
      default: return (
        <div className="panel" style={{ maxWidth: 500 }}>
          <div style={{ fontSize: 10, color: 'var(--text3)', letterSpacing: 2, marginBottom: 12 }}>
            {TITULOS[navActivo]?.toUpperCase()}
          </div>
  ...
  return (
    <div className="app-root" style={{ position: 'relative' }}>
      ...
      {/* Sidebar — fixed en móvil/tablet, relativo en desktop */}
      <div style={{
